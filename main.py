...
FIXED_SHARE_CHANNEL_CODE = "4"  # ✅ Biến mới
...
async def extract_origin_link_from_short(url: str) -> str:
    ...
    VD: https://s.shopee.vn/8ARWv9YbN0 → https://shopee.vn/product/123/456
    """
    try:
        print(f"\n{'='*60}")
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.get(
                ...
                # Nếu final URL có origin_link parameter, extract nó
                ...
                # Nếu final URL là link Shopee hợp lệ, return nó
...
def create_affiliate_link(origin_link: str) -> str:
    ...
    Format:
    https://s.shopee.vn/an_redir?origin_link=...&affiliate_id=...&sub_id=...&share_channel_code=4
    """
    ...
    affiliate_link = (
        ...
        f"&share_channel_code={FIXED_SHARE_CHANNEL_CODE}"  # ✅ Thêm biến mới
    )
...
async def create_link(origin_link: str = Query(...)):
    ...
    Process:
    1. Kiểm tra là short link hay không
    2. Nếu là short link → giải mã → lấy link gốc
    3. Tạo link affiliate mới với tất cả parameters
    """
    
    # ========== Validation ==========
    if not origin_link:
        raise HTTPException(status_code=400, detail="origin_link không được để trống")
    ...
    return {
        ...
        "subId": FIXED_SUB_ID,
        "shareChannelCode": FIXED_SHARE_CHANNEL_CODE  # ✅ Trả về trong response
    }
